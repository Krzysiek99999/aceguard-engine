...
# Schema 3: bet-size as a fraction of the pot. The subnet bot generator picks
# from a discrete pot-fraction menu (~{0.5, 0.66, 0.75, 1.0, 1.5}), verified on
# live bets, so menu-centered bins make bot sizing spikes visible. Ratios are
# scale-invariant, so this channel survives absolute amount/pot drift.
# 0=pad, 1=no-bet (check/fold/zero), 2=<0.35, 3=0.35-0.45, 4=~0.5, 5=~0.66,
# 6=~0.75, 7=~1.0, 8=~1.5, 9=>1.8
_POT_FRAC_VOCAB_SIZE = 10
_POT_FRAC_EDGES = (0.35, 0.45, 0.58, 0.70, 0.85, 1.15, 1.80)


def _pot_frac_id(amount_bb: float, pot_before_bb: float) -> int:
    if amount_bb <= 0.0 or pot_before_bb <= 1e-9:
        return 1
    frac = amount_bb / pot_before_bb
    for slot, edge in enumerate(_POT_FRAC_EDGES):
        if frac < edge:
            return slot + 2
    return 9
...
def encode_hand(
    ...
    *,
    ...
    pot_frac = np.zeros(max_actions_per_hand, dtype=np.int64)
    ...
    for idx in range(n_actions):
        ...
        pot_frac[idx] = _pot_frac_id(amount_bb, pot_before_bb)
...
def encode_chunk(
    ...
    *,
    ...
    pot_frac = np.zeros((max_hands_per_chunk, max_actions_per_hand), dtype=np.int64)
    ...
    for hand_idx, source_idx in enumerate(selected_indices):
        ...
        pot_frac[hand_idx] = encoded["pot_frac"]
...
class SequenceModelConfig:
    ...
    # Schema 3: schema 2 plus per-action pot-fraction bucket (bet/pot ratio,
    #           menu-centered bins; scale-invariant so independent of
    #           use_amount_features).
    ...
    # Live validator payloads currently expose much coarser amount/pot channels
    # than sanitized public benchmark units. Disable these for ablation models
    # that should lean on action order, streets, actor aliases, and hand meta.
    use_amount_features: bool = True
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "use_amount_features": bool(self.use_amount_features),
        }
...
class ChunkSetTransformer(nn.Module):
    ...
    def __init__(self, config: SequenceModelConfig) -> None:
        ...
        self.pot_frac_emb = nn.Embedding(_POT_FRAC_VOCAB_SIZE, d_model, padding_idx=0)
        ...
        self.action_encoder = nn.TransformerEncoder(
            action_layer, num_layers=config.n_action_layers, enable_nested_tensor=False
        )
        ...
        self.hand_encoder = nn.TransformerEncoder(
            hand_layer, num_layers=config.n_hand_layers, enable_nested_tensor=False
        )
    ...
    def forward(
        ...
        flat_pot_frac = pot_frac.reshape(batch * hands, actions)
        ...
        if self.config.use_amount_features:
            embed = (
                embed
        ...
        if int(self.config.schema_version) >= 3:
            # Pot-fraction is a ratio: scale-invariant, so it stays active even
            # with use_amount_features=False (drift-safe money signal).
            embed = embed + self.pot_frac_emb(flat_pot_frac)
...
class SequenceModelWrapper:
    ...
    # Optional warm start: a state_dict from a pretraining run (v130 spec
    # 4.3.4). Loaded into the fresh model at the start of fit(); the head
    # layers still train from the pretext-task values, which is fine because
    # fine-tuning updates all parameters.
    init_state_dict: Optional[Dict[str, Any]] = field(default=None, repr=False)
    ...
    def _new_model(self) -> ChunkSetTransformer:
        ...
        if self.init_state_dict:
            state = {
                key: (value.to(self.device) if hasattr(value, "to") else value)
                for key, value in self.init_state_dict.items()
            }
    ...
    def predict_proba(
        ...
        model = ChunkSetTransformer(self.config).to(self.device)
        state_dict = dict(self._model_state["state_dict"])
        # Artifacts saved before schema 3 lack pot_frac_emb weights. For
        # schema_version < 3 the forward pass never touches pot_frac_emb, so
        # backfill it from the fresh init instead of failing strict loading.
        missing = [key for key in model.state_dict() if key not in state_dict]
        if missing and all(key.startswith("pot_frac_emb.") for key in missing):
            fresh = model.state_dict()
            for key in missing:
                state_dict[key] = fresh[key]
