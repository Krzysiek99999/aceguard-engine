"""Inference for the full label-blind canonical hand-bag tree surface."""
...
VIEW_MODE = "hash_bag_full60"


def _validated_components(bundle: dict[str, Any]) -> list[dict[str, Any]]:
    if not isinstance(bundle, dict):
        raise TypeError("hash-bag bundle must be a dict")
    if str(bundle.get("feature_extractor") or "") != FEATURE_EXTRACTOR:
        raise ValueError("hash-bag feature extractor mismatch")
    if str(bundle.get("view_mode") or "") != VIEW_MODE:
        raise ValueError("hash-bag view mode mismatch")
    if str(bundle.get("blend_strategy") or "") != BLEND_STRATEGY:
        raise ValueError("hash-bag blend strategy mismatch")
    components = list(bundle.get("components") or [])
    if not components:
        raise ValueError("hash-bag bundle has no components")
    for component in components:
        keys = [str(key) for key in component.get("keys") or []]
        if not str(component.get("name") or "") or not keys or "model" not in component:
            raise ValueError("hash-bag component schema is invalid")
    return components
