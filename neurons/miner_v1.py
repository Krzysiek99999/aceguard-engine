...
def _variant_config(name: str) -> dict[str, Any]:
    ...
    v334_v11_consensus_lock7_dual25_top8 = False
    v373_original_hash_bag_top8 = False
    ...
    elif name.startswith("v321_"):
        prefix = "v321_"
        live_sized = True
        v321_original_handset_multiseed_top10 = True
    elif name.startswith("v323_"):
        prefix = "v323_"
        live_sized = True
    ...
    elif name.startswith("v334_"):
        prefix = "v334_"
        live_sized = True
        v334_v11_consensus_lock7_dual25_top8 = True
    elif name.startswith("v373_"):
        prefix = "v373_"
        live_sized = True
        v373_original_hash_bag_top8 = True
    ...
    if name.startswith(prefix):
        ...
        family = (
            ...
            else "v321_original_handset_multiseed_top10"
            ...
            else "v334_v11_consensus_lock7_dual25_top8"
            if v334_v11_consensus_lock7_dual25_top8
            else "v373_original_hash_bag_top8"
            if v373_original_hash_bag_top8
        ...
        return {
            ...
            "description": (
                (
                    ...
                    if v315_original_lambdamart_top10
                    else "Original AceGuard multiseed rank ensemble of three independently initialized permutation-invariant hand-set networks with a threshold-safe top-10 head."
                    if v321_original_handset_multiseed_top10
                    ...
                    if v324_v11_consensus_lock7_v321_top8
                    else "Five-signal v11 consensus protects seven members of the top-eight while a 75/25 dual-generation original hand-set ensemble orders the batch and fills one threshold position."
                    if v334_v11_consensus_lock7_dual25_top8
                    else "Original canonical hand-bag ensemble of independently regularized ranking and classification trees, served with a threshold-safe top-eight head."
                    if v373_original_hash_bag_top8
            ...
            "model_file": (
                ...
                if v315_original_lambdamart_top10
                else "data/models/v321_original_handset_multiseed_top10/model.pkl"
                if v321_original_handset_multiseed_top10
                ...
                if v324_v11_consensus_lock7_v321_top8
                else "data/models/v334_v11_consensus_lock7_dual25_top8/model.pkl"
                if v334_v11_consensus_lock7_dual25_top8
                else "data/models/v373_original_hash_bag_top8/model.pkl"
                if v373_original_hash_bag_top8
...
class Miner(BaseMinerNeuron):
    ...
    def _implementation_files(self) -> list[Path]:
        ...
        elif family == "v321_original_handset_multiseed_top10":
            files.extend(
                [
                    REPO_ROOT / "poker44" / "score" / "original_behavior_features.py",
        ...
        elif family == "v334_v11_consensus_lock7_dual25_top8":
            files.extend(
                [
                    REPO_ROOT / "poker44" / "score" / "v334_v11_consensus_lock_inference.py",
                    REPO_ROOT / "poker44" / "score" / "v323_v11_consensus_lock_inference.py",
                    REPO_ROOT / "poker44" / "score" / "ensemble_v11.py",
                    REPO_ROOT / "poker44" / "score" / "statistical_v5.py",
                    REPO_ROOT / "poker44" / "score" / "statistical_v6.py",
        ...
        elif family == "v373_original_hash_bag_top8":
            files.extend(
                [
                    REPO_ROOT / "poker44" / "score" / "model_view_hand_hash.py",
                    REPO_ROOT / "poker44" / "score" / "original_schema_contract_features.py",
                    REPO_ROOT / "poker44" / "score" / "sequence_schema.py",
                    REPO_ROOT / "poker44" / "score" / "temporal_consistency_features.py",
                    REPO_ROOT / "poker44" / "score" / "original_tree_surface_features.py",
                    REPO_ROOT / "poker44" / "score" / "chunk_view_aggregation.py",
                    REPO_ROOT / "poker44" / "score" / "original_tree_surface_inference.py",
                    REPO_ROOT / "poker44" / "score" / "original_redundancy_features.py",
                    REPO_ROOT / "poker44" / "score" / "original_hash_bag_features.py",
                    REPO_ROOT / "poker44" / "score" / "original_hash_bag_inference.py",
                    REPO_ROOT / self.variant_cfg["model_file"],
                    REPO_ROOT / "data" / "models" / family / "report.json",
                ]
            )
    ...
    def _build_manifest(self) -> dict[str, Any]:
        ...
        if family in {
            ...
            if family in {
                ...
                training_statement = (
                    "Original AceGuard multiseed rank ensemble of three independently initialized "
                    "permutation-invariant hand-set networks trained only "
                    "on miner-visible public Poker44 benchmark releases through sourceDate "
                    "2026-06-30. It independently encodes canonical action, hero response, "
                    "pot geometry, position, and stack features for each hand, contextualizes "
                    "the unordered set without positional encoding, combines component ranks with "
                    "weights 0.4/0.3/0.3, and serves a threshold-safe top-10 rank head. Unlabeled "
                    "miner payloads through "
                    "2026-07-05T23:59:59Z were used only for per-hand train/live KS filtering. "
                    "Architecture, weights and top-10 were selected on July 6-7, evaluated once on "
                    "July 8-10, then blindly evaluated on the July 11 release without tuning. No "
                    "competitor implementation or "
                    "weights, private labels, identities, cards, outcomes, wallets, hotkeys, "
                    "IP addresses, or private data were used."
                )
            elif family == "v323_v11_consensus_lock8_v321_top10":
                training_statement = (
                    "Original AceGuard confidence-aware hybrid. Five independently implemented "
                    "v11 behavioural signals select eight protected members from the v11 top-10 "
                    "in each 100-chunk batch. The original v321 three-seed permutation-invariant "
                    "hand-set ensemble orders the protected chunks and all remaining chunks, "
                    "filling the other two threshold positions. V321 was trained only on "
                    "miner-visible public Poker44 benchmark releases through sourceDate "
                    "2026-06-30. July 6-7 selected the topology, July 8-10 was the temporal test, "
                    "and July 11 is disclosed only as a post-release diagnostic. Three independent "
                    "sampler seeds selected the same topology. Unlabeled live payloads were used "
                    "only for overlap, schema, stability, and runtime checks. No competitor code "
                ...
                training_statement = (
                    "Original AceGuard v11 single-replacement hybrid with a five-network "
                    "dual-generation permutation-invariant hand-set challenger. Three components "
                    "were trained through sourceDate 2026-06-30 and two through 2026-07-07. "
                    "The disclosed 75/25 rank blend, lock7/head8 and top8 operating point were "
                    "selected post-hoc on July 8-11 robustness and unlabeled topology checks and "
                    "therefore require live canary validation. No competitor weights, private "
                    "labels, identities, cards, outcomes, wallets, hotkeys, IP addresses, or "
                    "private data were used."
                )
            elif family == "v373_original_hash_bag_top8":
                training_statement = (
                    "Original AceGuard canonical hand-bag ensemble trained from scratch only on "
                    "official split=train public Poker44 canonical miner-visible API SourceUnits "
                    "through sourceDate 2026-07-11. One feature row is one natural SourceUnit; "
                    "players, chunks, and source units are never concatenated, and the validator "
                    "sanitizer is not reapplied. The fixed ensemble combines a LightGBM ranking "
                    "surface, ExtraTrees motif/redundancy surface, and HistGradientBoosting surface "
                    "using within-batch percentile ranks. Five rolling temporal folds fit feature "
                    "selection and models only on earlier dates. Top-eight and all model bytes were "
                    "frozen before the one-shot 2026-07-12 blind, which passed without retraining. "
                    "No competitor code or weights, private labels, identities, cards, outcomes, "
                    "wallets, hotkeys, IP addresses, or private live data were used."
                )
        ...
        manifest = build_local_model_manifest(
            ...
            defaults={
                ...
                "model_version": os.getenv(
                    ...
                    else "2026.07.11-v334-v11-consensus-lock7-dual25-top8"
                    if family == "v334_v11_consensus_lock7_dual25_top8"
                    else "2026.07.12-v373-canonical-blind-pass-top8"
                    if family == "v373_original_hash_bag_top8"
        ...
        if family == "v321_original_handset_multiseed_top10":
            ...
            manifest["training_refresh"] = "v321_original_multiseed_blind_validated_2026-07-11"
        if family == "v323_v11_consensus_lock8_v321_top10":
            manifest["training_data_sources"] = [
                "https://api.poker44.net/api/v1/benchmark",
                "https://api.poker44.net/api/v1/benchmark/chunks?sourceDate=2026-06-30",
        ...
        if family == "v334_v11_consensus_lock7_dual25_top8":
            ...
            manifest["training_refresh"] = "v334_v11_consensus_lock7_dual25_head8_top8_candidate_2026-07-11"
        if family == "v373_original_hash_bag_top8":
            manifest["training_data_sources"] = [
                "https://api.poker44.net/api/v1/benchmark",
                "https://api.poker44.net/api/v1/benchmark/chunks?sourceDate=2026-07-11",
            ]
            manifest["training_refresh"] = "v373_canonical_frozen_blind_passed_2026-07-12"
    ...
    def _score_v298_lambdamart_wide_model(self, chunks: list[list[dict[str, Any]]]) -> list[float]:
        ...
        is_v334 = family == "v334_v11_consensus_lock7_dual25_top8"
        is_v373 = family == "v373_original_hash_bag_top8"
        if is_v373:
            from poker44.score.original_hash_bag_inference import load_bundle, score_chunks
        elif is_v334:
            from poker44.score.v334_v11_consensus_lock_inference import load_bundle, score_chunks
        elif is_v324:
