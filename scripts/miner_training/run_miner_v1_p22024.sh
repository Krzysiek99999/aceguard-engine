#!/usr/bin/env bash
# UID 255 (port 22024) — poker44-v14-rank-top2-uid255
# Wave 1 swap (2026-05-27): v8_markov (dup) → v14 LightGBM + rank_cap top2.
# Live std=0.22, pearson ~-0.05 vs UID 160 (orthogonal). 5% bot rate per window.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
export POKER44_V1_VARIANT="v14_rank_top2"
export POKER44_MODEL_NAME="poker44-v14-rank-top2-uid255"
...
export POKER44_MODEL_TRAINING_DATA_STATEMENT="LightGBM v14 trained on public benchmark (2026-04-30..05-06 train, 05-07 val, 05-08 holdout) with KS-filter (live-distribution-stable 21 features) + rank_cap_remap top-2 binarization. Hold-out AP=0.79 FPR=0.04 reward=0.67. Live std=0.22."
...
# v14_rank_top2: rank_cap_remap top_n=2 w dispatch, brak adaptive_n / max_bot_fraction.

exec .venv/bin/python neurons/miner_v1.py \
    --netuid 126 \
    --wallet.name poker44_p22018 \
    --wallet.hotkey poker44_super_a \
    --subtensor.network finney \
    --axon.port 22024 \
    --axon.external_ip 80.238.120.82 \
    --axon.external_port 22024 \
    "$LOG_FLAG" \
