#!/usr/bin/env bash
# UID 190 (port 22021) — poker44-v19-rank-top2-uid190
# Wave v19-B (2026-05-28): baseline_heuristic (0/40 zero signal) → v19_rank_top2.
# Medium prior between UID 89 (top1) and UID 252 (top3). Same v19 LambdaRank model.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
export POKER44_V1_VARIANT="v19_rank_top2"
export POKER44_MODEL_NAME="poker44-v19-rank-top2-uid190"
...
export POKER44_MODEL_TRAINING_DATA_STATEMENT="LightGBM LambdaRank trained on Poker44 public benchmark API shadow-training-v1 release (2026-05-26, 90 chunks, 6 release groups, 50/50 balanced synthetic bots vs human anchors). Leave-one-release-out cross-validation OOF AP=0.84. Rank-cap remap top-2 binarization per 40-chunk window."
...
# v19_rank_top2: deterministic rank_cap_remap top_n=2 in dispatch, no adaptive_n.

exec .venv/bin/python neurons/miner_v1.py \
    --netuid 126 \
    --wallet.name poker44_p22021 \
    --wallet.hotkey poker44_p22021_hot \
    --subtensor.network finney \
    --axon.port 22021 \
    --axon.external_ip 80.238.120.82 \
    --axon.external_port 22021 \
    "$LOG_FLAG" \
