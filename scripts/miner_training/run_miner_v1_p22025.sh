#!/usr/bin/env bash
# UID 252 (port 22025) — poker44-v19-rank-top3-uid252
# Wave v19-A (2026-05-28): v11_ensemble → v19 LambdaRank top3.
# Medium slot: 3 boty/40 per request (najbliżej zwycięskiego trybu UID160/v5).
set -euo pipefail
cd "$(dirname "$0")/../.."
...
export POKER44_V1_VARIANT="v19_rank_top3"
export POKER44_MODEL_NAME="poker44-v19-rank-top3-uid252"
...
export POKER44_MODEL_TRAINING_DATA_STATEMENT="LightGBM LambdaRank trained on Poker44 public benchmark API shadow-training-v1 release (2026-05-26, 90 chunks, 6 release groups, 50/50 balanced synthetic bots vs human anchors). Leave-one-release-out cross-validation OOF AP=0.84. Rank-cap remap top-3 binarization per 40-chunk window."
...
# v19_rank_top3: deterministic rank_cap_remap top_n=3 in dispatch, NO adaptive_n / cap envs.

exec .venv/bin/python neurons/miner_v1.py \
    --netuid 126 \
    --wallet.name poker44_p22020 \
    --wallet.hotkey poker44_real_b \
    --subtensor.network finney \
    --axon.port 22025 \
    --axon.external_ip 80.238.120.82 \
    --axon.external_port 22025 \
    "$LOG_FLAG" \
